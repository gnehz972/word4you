...
impl ConfigManager {
    ...
    pub fn run_setup(term: &Term) -> Result<()> {
        term.write_line(
            &style("🔧 Word4You Configuration Setup")
                .cyan()
                .bold()
                .to_string(),
        )?;
        ...
        if config.ai_provider == "gemini" {
            ...
            let gemini_api_key = if !config.gemini_api_key.is_empty() {
                ...
                if Confirm::new()
                    .with_prompt(format!(
                        "Current Gemini API key: {}. Update it?",
                        masked_key
                    ))
                    .default(false)
                    .interact()?
        ...
        if git_enabled {
            ...
            config.git_remote_url = if git_url.is_empty() {
                None
            } else {
                Some(git_url)
            };
    ...
    pub fn test_config(term: &Term) -> Result<()> {
        if !Self::config_exists() {
            return Err(anyhow!(
                "Configuration not found. Run 'word4you config' to set up."
            ));
        }
...
"#;
        ...
        assert_eq!(
            migrated_config.git_remote_url,
